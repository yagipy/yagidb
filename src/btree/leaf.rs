use std::mem::size_of;
use zerocopy::{AsBytes, ByteSlice, FromBytes, LayoutVerified, ByteSliceMut};
...
impl<B: ByteSlice> Leaf<B> {
    pub fn new(bytes: B) -> Self {
        let (header, body) = LayoutVerified::new_from_prefix(bytes).expect("leaf page must be aligned");
