...
use zerocopy::{ByteSlice, AsBytes};
...
impl BTree {
    pub fn create(buffer_pool_manager: &mut BufferPoolManager) -> Result<Self, Error> {
        let meta_buffer = buffer_pool_manager.create_page()?;
        let mut meta = meta::Meta::new(meta_buffer.page.borrow_mut() as RefMut<[_]>);
        let root_buffer = buffer_pool_manager.create_page()?;
    ...
    fn fetch_root_page(&self, buffer_pool_manager: &mut BufferPoolManager) -> Result<Rc<Buffer>, Error> {
        let root_page_id = {
            let meta_buffer = buffer_pool_manager.fetch_page(self.meta_page_id)?;
            let meta = meta::Meta::new(meta_buffer.page.borrow() as Ref<[_]>);
            meta.header.root_page_id
        };
        Ok(buffer_pool_manager.fetch_page(root_page_id)?)
    }
    ...
    fn search_internal(
        ...
        match node::Body::new(node.header.node_type, node.body.as_bytes()) {
            node::Body::Leaf(leaf) => {
                ...
                if is_right_most {
                    iter.advance(buffer_pool_manager)?;
                }
                Ok(iter)
            }
            node::Body::Branch(branch) => {
                ...
                drop(node);
                drop(node_buffer);
                let child_node_page = buffer_pool_manager.fetch_page(child_page_id)?;
                self.search_internal(buffer_pool_manager, child_node_page, search_mode)
            }
        }
    }
    ...
    pub fn search(
        ...
        let root_page = self.fetch_root_page(buffer_pool_manager)?;
        self.search_internal(buffer_pool_manager, root_page, search_mode)
    }
        ...
        match node::Body::new(node.header.node_type, node.body) {
            node::Body::Leaf(mut leaf) => {
                let slot_id = match leaf.search_slot_id(key) {
                    Ok(_) => return Err(Error::new(ErrorKind::Other, "duplicate key")), // TODO: 修正
                    Err(slot_id) => slot_id,
                };
                if leaf.insert(slot_id, key, value).is_some() {
                    ...
                    let prev_leaf_buffer = prev_leaf_page_id
                        .map(|next_leaf_page_id| buffer_pool_manager.fetch_page(next_leaf_page_id))
                        .transpose()?;

                    let new_leaf_buffer = buffer_pool_manager.create_page()?;
            ...
            node::Body::Branch(mut branch) => {
                ...
                let child_node_buffer = buffer_pool_manager.fetch_page(child_page_id)?;
                if let Some((overflow_key_from_child, overflow_child_page_id)) =
                    self.insert_internal(buffer_pool_manager, child_node_buffer, key, value)?
                {
                    if branch
                        ...
                        .is_some()
                    {
                        ...
                        let new_branch_buffer = buffer_pool_manager.create_page()?;
    ...
    pub fn insert(
        ...
        let root_buffer = buffer_pool_manager.fetch_page(root_page_id)?;

        if let Some((key, child_page_id)) = self.insert_internal(buffer_pool_manager, root_buffer, key, value)? {
            let new_root_buffer = buffer_pool_manager.create_page()?;
...
impl Iter {
    ...
    fn advance(&mut self, buffer_pool_manager: &mut BufferPoolManager) -> Result<(), Error> {
        self.slot_id += 1;
        let next_page_id = {
            let leaf_node = node::Node::new(self.buffer.page.borrow() as Ref<[_]>);
            let leaf = leaf::Leaf::new(leaf_node.body);
        ...
        if let Some(next_page_id) = next_page_id {
            self.buffer = buffer_pool_manager.fetch_page(next_page_id)?;
            self.slot_id = 0;
        }
        Ok(())
    }
    ...
    pub fn next(
        ...
        let value = self.get();
        self.advance(buffer_pool_manager)?;
        Ok(value)
    }
}
